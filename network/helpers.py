...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None):
    ...
    if in_chans == 1:
        ...
        conv1_name = cfg['first_conv']
        logging.info('Converting first conv (%s) from 3 to 5 channel' % conv1_name)
        conv1_weight = state_dict[conv1_name + '.weight']
        state_dict[conv1_name + '.weight'] = torch.cat([conv1_weight,conv1_weight.sum(dim=1, keepdim=True),conv1_weight.sum(dim=1, keepdim=True)],dim=1)
    elif in_chans != 3:
